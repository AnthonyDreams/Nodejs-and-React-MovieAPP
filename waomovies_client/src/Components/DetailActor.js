...
import MovieGrid from "../Containers/BasicCarousel";
...
export default function DetailActor({ actor, onClose }) {
  ...
  const fetchData = async () => {
    const result = await makeQuery(() => MovieApi.actors.get(actor));
    setData(result.data);
    setShow(false)
    setExcludeIds(result.data.interpretations.map(interpretation=> interpretation.movie.id))
    setInterpretation({ actor: { id: result.data.id } });
  };
            ...
            <Grid container direction="row" justify={"space-between"} spacing={7}>
              ...
              <Grid container item xs={8} spacing={6}>
                ...
                <Grid item xs={12}>
                  ...
                  <br></br>
                  <MovieGrid
                    data={data.interpretations.map(
                      (interpretation) => interpretation.movie
                    )}
                  />
                </Grid>
                <Grid item xs={12}>
                  ...
                  <Grid item xs={12}>
                    <Grid container spacing={3} direction={"row"}>
                      <AutoComplete
                        fetchFunction={MovieApi.movies.getAll}
                        entity={"Movie"}
                      ...
                      <Button
                        variant="contained"
                        color="primary"
                        disabled={!interpretation.movie  || !interpretation.movie.id}
                    ...
                    <Grid item>
                      <CharacterList fetchData={fetchData} data={data.interpretations}/>
